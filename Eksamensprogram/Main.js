...
async function Træk(x, y) {
  let valgtTræk = gyldigeTræk.find(træk => træk.x === x && træk.y === y);
  ...
  if (selectedPiece.type === "konge"|| selectedPiece.type === "tårn") {
    await Rokade();
  }
...
//visuelt skal nok ind i en anden fil
function tegnBræt() {
...
function SkakStatus(bræt, farve) {
 ...
 opdaterBræt();//skal måske ændres da den lige nu kun er relevant når man refresher siden
